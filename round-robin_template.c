    ...
    */
    ...
    while (finished_processes < num_processes) {
        if (!is_empty(queue)) {
            ...
            int idx = cur_process.id; // Get the index of the current process in the processes array

            // Determine the time slice for the current process
            int time_slice = (processes[idx].remaining_burst_time > quantum) ? quantum : processes[idx].remaining_burst_time;

            // time slice
            run_process(&cur_time, time_slice, &processes[idx]);
            ...
            if (processes[idx].remaining_burst_time <= 0) {
                // Process completed
                processes[idx].completion_time = cur_time;
                finished_processes++;
            } else {
                // Re-enqueue the process if it's not completed
                enqueue(queue, processes[idx]);
            }
        } else {
            // If the queue is empty, increment current time
            cur_time++;
        }
        
        // Check for new arrivals at the updated current time
        check_arrival_time(queue, processes, cur_time, num_processes);
    }
...
void calculate_metrics(Process *processes, int num_processes) {
    ...
    float total_waiting_time, total_turnaround_time = 0.0;
